...
import com.india.womansafety_2024.databinding.ActivityMapsBinding;
import com.india.womansafety_2024.model.UserLatLng;
import com.india.womansafety_2024.utils.KeyBoardUtils;
import com.india.womansafety_2024.utils.NetworkUtils;
...
// Construct a CameraPosition focusing on Mountain View and animate the camera to that position.
    ...
    private void getRecords(String mobiile) {
        ...
        mFireBaseData = FirebaseDatabase.getInstance(FirebaseApp.initializeApp(MapsActivity.this)).getReference(getString(R.string.str_user)).child(mobiile).child(getString(R.string.str_tracking));
        ...
        mFireBaseData.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (snapshot.exists()) {
                    ...
                    addMarkers();  // with help of locationList add Markers on map
                } else {
    ...
    public void displaySnackBarWithButton() {
        try {
            ...
            final Snackbar mSnackbar = Snackbar.make(mBinding.parentLayout, getString(R.string.str_clear_track), Snackbar.LENGTH_LONG)
                    .setAction("Yes", v -> {
                        mBinding.progressbarMap.setVisibility(View.VISIBLE);
                        mFireBaseData = FirebaseDatabase.getInstance(FirebaseApp.initializeApp(MapsActivity.this)).getReference(getString(R.string.str_user)).child(mobNO).child(getString(R.string.str_tracking));

                        mFireBaseData.removeValue().addOnSuccessListener(aVoid -> {  // to delete all values from tracking table
